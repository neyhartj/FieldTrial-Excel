...
design2xlsx <- function (
  ...
  if (!is.null(CheckCol)) {
    ...
    if (length(CheckCol) != max(design[[1]][[1]]$line_code)) stop("The CheckCol vector is not of the same length as the number of checks")
  ...
  if (!is.null(SheetNames)) {
    ...
    if (length(SheetNames) != length(design)) stop("The length of the SheetNames vector is not the same as the length of the x list input")
  ...
  if (!is.null(SheetTitle)) {
    if (length(SheetTitle != legnth(design))) stop("The SheetTitle vector is not of the same length as the x list input")
  ...
  if (is.null(SheetTitle)) {
    for (l in 1:length(design)) {
    SheetTitle <- as.character(unique(design[[1]]$trial))
    }
  }
  ...
  names(design[[1]]) <- sub(pattern = "_", replacement = ".", x = names(design[[1]]))
  ...
  if (length(grep(pattern = "Row", x = design[[3]][,1][1])) == 0) {
    design[[3]][,1] <- c(paste("Row",design[[3]][-length(design[[3]][,1]),1], sep = " "),"Columns:")
  }
  #Extract the checks
  ...
  design_tempdf <- design[[1]][,match(x = design_dfnames[DgnNames], table = names(design[[1]]))[which(!is.na(x = match(x = design_dfnames[DgnNames], table = names(design[[1]]))))]]
  #Add names to the data.frame
  names(design_tempdf)[1:dim(design_tempdf)[2]] <- names_vec[match(x = names(design_tempdf)[1:dim(design_tempdf)[2]], table = names(design[[1]]))]
  ...
  if (!is.na(match(12, DgnNames))) {
    ...
    for (r in 1:(dim(design[[3]])[1]-1)) { #For loop to cycle through the number of rows
      for (k in 1:(dim(design[[3]])[2]-1)) { #For loop to cycle through the number of columns
        ...
        if (sum(design[[1]]$line.name[which(design[[1]]$plot == pf_value)] == checks_df[,2]) == 1) {
          setCellStyle(cell = pf_cell[[1]], cellStyle = CS + Fill(foregroundColor = CheckCol[which(design[[1]]$line.name[which(design[[1]]$plot == pf_value)] == checks_df[,2])], backgroundColor = "white"))  
  ...
  if (BlkBorders == TRUE) {
    ...
    if (substring(text = design.type, first = 1, last = gregexpr(pattern = ".dgn", text = design.type)[[1]][1]-1) == "aibd" || substring(text = design.type, first = 1, last = gregexpr(pattern = ".dgn", text = design.type)[[1]][1]-1) == "mad") {
      ...
      blk_dim <- numeric()
      blk_dim[1] <- max(design[[1]]$row)/max(design[[1]][,9]) #Add the number of rows in each block
      blk_dim[2] <- max(design[[1]]$column)/max(design[[1]][,10]) #Add the column dimension (this is done by finding various maxima and minima in the dgn data.frame)
      
      for (r in 1:max(design[[1]][,9])) { #For loop to cycle through the number of row_blk
        
        for (k in 1:max(design[[1]][,10])) { #For loop to cycle through the number of col_blk
        ...
        if (substring(text = design.type, first = 1, last = gregexpr(pattern = ".dgn", text = design.type)[[1]][1]-1) == "rcbd") {
          ...
          cellblk1 <- CellBlock(sheet = main.sheet, #Designate the sheet
                                ...
                                noColumns = (dim(design[[3]]) - 1)[2], #Number of cols is the number of cols in the field design
                                create = FALSE)
          ...
          CB.setBorder(cellBlock = cellblk1, border = Border(position = c("TOP", "LEFT"), pen = "BORDER_MEDIUM"), rowIndex = 1, colIndex = 1)
          #Top right
          CB.setBorder(cellBlock = cellblk1, border = Border(position = c("TOP", "RIGHT"), pen = "BORDER_MEDIUM"), rowIndex = 1, colIndex = (dim(design[[3]]) - 1)[2])
          #Bottom right
          CB.setBorder(cellBlock = cellblk1, border = Border(position = c("BOTTOM", "RIGHT"), pen = "BORDER_MEDIUM"), rowIndex = (dim(design[[3]]) - 1)[1], colIndex = (dim(design[[3]]) - 1)[2])
          #Bottom left
          CB.setBorder(cellBlock = cellblk1, border = Border(position = c("BOTTOM", "LEFT"), pen = "BORDER_MEDIUM"), rowIndex = (dim(design[[3]]) - 1)[1], colIndex = 1)
          #Top row
          CB.setBorder(cellBlock = cellblk1, border = Border(position = c("TOP"), pen = "BORDER_MEDIUM"), rowIndex = 1, colIndex = 2:((dim(design[[3]]) - 1)[2]-1)) #Columns are added by starting at the second column (2), then including all columns up until the last one (blk_dim[2]-1)
          #Right side
          CB.setBorder(cellBlock = cellblk1, border = Border(position = c("RIGHT"), pen = "BORDER_MEDIUM"), rowIndex = 2:((dim(design[[3]]) - 1)[1]-1), colIndex = (dim(design[[3]]) - 1)[2]) #Rows are added by starting at the second row (2), then including all rows up until the last one (blk_dim[1]-1)
          #Bottom row
          CB.setBorder(cellBlock = cellblk1, border = Border(position = c("BOTTOM"), pen = "BORDER_MEDIUM"), rowIndex = (dim(design[[3]]) - 1)[1], colIndex = 2:((dim(design[[3]]) - 1)[2]-1))
          #Left side
          CB.setBorder(cellBlock = cellblk1, border = Border(position = c("LEFT"), pen = "BORDER_MEDIUM"), rowIndex = 2:((dim(design[[3]]) - 1)[1]-1), colIndex = 1)
          ...
          for (r in 1:max(design[[1]]$row)) { #For loop to cycle through the number of rows
